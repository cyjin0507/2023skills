...
<section id="reservation" class="mt-5 content">
    ...
    <div class="between mt-5">
        ...
        <button class="btn btn-success" style="font-size: 15px;">예약의 총 건수 : <span style="font-size: 25px;"><?=count($reservation)?></span>건</button>
        <button class="btn btn-success" style="font-size: 15px;">합계금액 : <span style="font-size: 25px;"><?=number_format($reservationTotalCount)?></span>원</button>
    </div>
...
<div class="title">
    ...
    document.querySelector('input[name="orderdate"]').addEventListener('change', (e)=> setTable(e.target.value))
        ...
        function createTr({
            ...
            tr.innerHTML = `
                <tr>
                    <td>${date} / ${order.reduce((s, order)=>(order.create_date), '주문하지 않았습니다.')}</td>
                    <td>${place}</td>
                    <td class="order-count">${order.reduce((s,order)=>(order.type!=='cancel'?s+1:s),0)}건</td>
                    <td>${order.reduce((s,order)=>(order.type=="complete"?s+1:s),0)}건</td>
                    <td>${order.reduce((s,order)=>(order.type=="complete"?parseInt(s+(order.price)).toLocaleString(undefined):s),0)}원</td>
                    <td><button class="btn look">주문상세보기</button></td>
                <tr>
            `
        ...
        function setTotal() {
            ...
            document.querySelector('.order-four').innerHTML =parseInt(currentReservationList.reduce((s, {
                order
            })=>
            s + order.reduce((s,order)=> (order.type == 'complete' ? s+(order.price) : s),0),0)).toLocaleString(undefined)
        }
