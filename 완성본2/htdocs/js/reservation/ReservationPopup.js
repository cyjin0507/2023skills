class ReservationPopup extends Popup {
    ...
    addEvent() {
        ...
        this.reservationBtn.addEventListener('click', async ()=> {
            ...
            const response = await fetch('./process_reservation_add.php',{
                method : "post",
                headers : {"Content-Type" : "application/json"},
                body : JSON.stringify({
                    name, phone, price:this.price, date : this.date.myFormat(), place:this.spaceName
                })
            }) 

            // this.document.querySelector('.show-txt').innerHTML = await response.text()
            ...
            this.thisWindow.alert('예약정보가 정상 등록되었습니다. 관리자 승인 후 예약이 최종 완료됩니다.')
            this.thisWindow.close()
